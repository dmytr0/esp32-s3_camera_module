...
{
    size_t b_write = 0;
    uint8_t *data_ptr = (uint8_t *)pcm_start;
    ...
    while(1)
    {
        ...
        if(b_write>0)
        {
             printf("[music] i2s music played, %d bytes are written.", b_write);
        }
        else {
           printf("[music] i2s music play failed.");
        }
        data_ptr = (uint8_t *)pcm_start;
